class PerseveranceScraper
  ...
  attr_reader :rover
  def initialize
    @rover = Rover.find_by(name: "Ingenuity")
  end
  ...
  def collect_links
    response = (URI.open("https://mars.nasa.gov/rss/api/?feed=raw_images&category=ingenuity&feedtype=json&latest=true").read)
    ...
    sols_to_scrape.map { |sol|
	    "https://mars.nasa.gov/rss/api/?feed=raw_images&category=ingenuity&feedtype=json&sol=#{sol}"
    }
  end
